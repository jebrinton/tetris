    ...
    extends Agent
{
        ...
        extends Object
    {
        ...
        public Coordinate getNextCoordinateToExplore()
        {
            ...
            // Calculate the total possible ways to have pits
            // Exactly one of these is the true state, so the sum of their probabilities is 1
            HashMap<Coordinate, Boolean> totalPossible = new HashMap<>();
            for (Coordinate coordinate : frontierPitCoordinates) {
                totalPossible.put(coordinate, true);
                totalPossible.put(coordinate, false);
            }

            System.out.println(totalPossible);

            // Go through the total possible ways and check if they're possible given what we know
            // Keep a count of the probability sum of these occurring 
